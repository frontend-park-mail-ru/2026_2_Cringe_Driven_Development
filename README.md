...
Все фронтовые задачи команды живут на одной [доске](https://github.com/orgs/Cringe-Driven-Development-Team/projects/1)
...
2. **Создать ветку.** Открыть issue → в правой колонке кнопка
   **`Create a branch for this issue`**. GitHub предложит имя, собранное из заголовка
   задачи, — заменить его на `cdd-<номер issue>`, например `cdd-12`.
...
4. **Открыть pull request** в `main`, когда код готов к ревью.
   Заголовок — по шаблону `CDD-<номер issue>: <название issue>`, например
   `CDD-12: Сделать фор`.
